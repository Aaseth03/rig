...
| `hooks/` | Hook scripts, each paired with a `<name>.hook.json` descriptor — see [hooks/CONTEXT.md](hooks/CONTEXT.md) |
...
- **Add a new hook** → drop a `<name>.py`/`.sh` plus `<name>.hook.json` into `hooks/`, see [hooks/CONTEXT.md](hooks/CONTEXT.md)
